...
require(ggpubr)
library(VennDiagram)
#This script achieve the analysis for expression data of candidate gene #
#https://soulnature.github.io/brainepl/Risk_gene.html#
#promoter_signal.matrix were compute by ourself, if you need, you can pull a request by email
gene_list_us_enrichment<-read.table('candidate gene.txt',fill = TRUE,sep = '\t',header = TRUE  )
...
###
test_data<-c('parietal lobe, fetal','parietal lobe, adult','parietal lobe,newborn')
...
plot_brain_region_diff(c('parietal lobe, fetal','parietal lobe, adult'))
plot_brain_region_diff(c('substantia nigra, newborn','substantia nigra, adult'))
###
graph2ppt(file='region_exp.pptx',width=8,height=6,append=TRUE)
